setwd("/share/hennlab/data/SA_sim/sim/khm/4pop_complex/")
...
SumStat <- function(TrajFile){
  ...
  tEP <- as.numeric(name.seg[[1]][4])
  mPK <- as.numeric(name.seg[[1]][6])
  tPK <- as.numeric(name.seg[[1]][8])
  rm(name.seg)
  
  Ne = 20000  # current Ne of KhoeSan #!
  ...
  nanc.K <- 269*2 - 175 #number of ancestral copies in KhoeSan #!
  nsel.K <- 175 #!
  nanc.P <-  100-38
  nsel.P <- 38
  nanc.E <- 0
  ...
  nanc.B <- 198
  nsel.B <- 0
  nanc.total <-nanc.K +nanc.P +nanc.E +nanc.B
  nsel.total <- nsel.K + nsel.P + nsel.E + nsel.B
  ...
  #run mssel  
  system(paste("/share/hennlab/progs/Meng_mssel_programs/mssel ", nsam.total, " 1 ", nanc.total, " ", nsel.total, " /share/hennlab/data/SA_sim/trajectories/khm/Khm_4popComplex_traj/", TrajFile, 
  " 23316 -r ", 4*Ne*31700*r.rate, " 31700 -t ", 4*Ne*31700*mu, " -I 4 ", nanc.K, " ", nsel.K, " ", nanc.P, " ", nsel.P, " ", nanc.E, " ", nsel.E, " ", nanc.B, " ", nsel.B,
  " -n 1 1.0 -n 2 ", NP/Ne, " -n 3 ", NE/Ne, " -n 4 ", NB/Ne, 
  " -es ", tEK/4/Ne, " 1 ", 1-mEK, " -ej ", tEK/4/Ne, " 5 3 -en ", tEK/4/Ne, " 3 ", NE/Ne,
  " -es ", tBK/4/Ne, " 1 ", 1-mBK, " -ej ", tBK/4/Ne, " 6 4 -en ", tEK/4/Ne, " 4 ", NB/Ne,
  #" -en 0.0005 1 2.0", #nama bottleneck #! 
  " -es ", tPK/4/Ne, " 1 ", 1-mPK, " -ej ", tPK/4/Ne, " 7 2 -en ", tPK/4/Ne, " 2 ", NP/Ne,
  " -es ", tEP/4/Ne, " 2 0.7 -ej ", tEP/4/Ne, " 8 3 -en ", tEP/4/Ne, " 3 ", NE/Ne,
  " -en ", 1800/4/Ne, " 3 ", 1000/Ne,
  " -ej ", 65000/30/4/Ne, " 3 2",
  " -ej ", 75000/30/4/Ne, " 2 4",
  " -ej ", 110000/30/4/Ne, " 4 1 > ", TrajFile, ".mssel", sep=""))
   
  
  
  
  
  #split mssel to San derived and ancestral for calc on s and pi, EHH not affected
  ms.all <- read.table(paste(TrajFile,".mssel",sep=""), sep = "", header = F, colClasses = "character", skip = 8)
  ms.output<- as.matrix(ms.all[c(c(1:(nanc.K +nsel.K)), nanc.K + nsel.K + nanc.P + nsel.P + 10),]) # Keep San haplotypes, and one European haplotype as reference
  ...
  return(c(sel/2, pi,s,ihh, ehh.up, ehh.down, tEP, mPK, tPK, r.rate))
}
...
seg.size <- 50 # each job takes care of 50 trajectories
...
sum.stats <- matrix(nrow=10*nrow(traj.list),ncol=10)
colnames(sum.stats) <- c("s2", "pi", "ss","ihh","ehh.up","ehh.down", "tEP","mPK","tPK", "RecRate")
